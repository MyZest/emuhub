...
ARCH=$(uname -m)
if [ "$ARCH" = "x86_64" ]; then
    PACKAGE="system-images;android-34;google-tv;x86"
    GPU_FLAG="-gpu host"
else
    echo "当前为 ARM 主机，Android TV 暂无 arm64-v8a 官方系统镜像，建议在 x86_64 主机运行。" >&2
    exit 1
fi
...
if /opt/android-sdk-linux/cmdline-tools/tools/bin/avdmanager list avd | grep -q $DEVICE_NAME; then
    ...
    echo no | /opt/android-sdk-linux/cmdline-tools/tools/bin/avdmanager create avd --name $DEVICE_NAME --device $DEVICE_ID --package $PACKAGE 
    ...
    sed -i 's/hw\.initialOrientation\s*=\s*portrait/hw.initialOrientation = landscape/' $AVD_DIR/config.ini
